<div class="min-w-96 basis-1/3 flex-1 max-w-1/2 min-h-48 max-h-80">
    <div class="items-center px-2 min-h-[150px] w-full bg-white md:flex-row">
        <img class="float-left pr-2 w-[150px] max-w-full h-[150px] max-h-full object-cover" src="{{ $post->thumbnail ? asset('storage/' . $post->thumbnail) : '/images/o1.png'}}" alt="">
        <div class="flex flex-col justify-center flex-wrap p-4 leading-normal text-sm">
            <h5 class="mb-2 font-bold tracking-tight text-base text-blue-700 hover:text-red-500"><a href="posts/{{ $post->slug }}">{{ $post->title}}</a></h5>
