...
<x-layout>
    ...
    <main class="flex mx-20 px-20">
        <x-menu/>
        <div class="flex-start py-10 space-y-10">
            ...
            <div class="flex row font-semibold">
                <div class="{{!isset($previous_record) ? 'hidden' : ''}}bg-gray-100 w-1/2 p-4 text-left text-blue-600 transition duration-300 hover:bg-blue-600 hover:text-white">
                    @if (isset($previous_record))
                        <a href="/posts/{{ $previous_record->slug }}" class="h-full w-full">
                            <p>
                ...
                <div class="{{!isset($next_record) ? 'hidden' : ''}} bg-gray-100 w-1/2 p-4 text-right text-blue-600 transition duration-300 hover:bg-blue-600 hover:text-white">
                    @if (isset($next_record))
                        <a href="/posts/{{ $next_record->slug }}" class="h-full w-full">
                            <p class="inline">
                                {{ $next_record->title }}
                                <svg class="w-3 h-3 inline mb-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 10 16">
